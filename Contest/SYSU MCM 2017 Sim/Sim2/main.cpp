...
*/
int createMatrix(vector<vector<double> >& m, vector<vector<double> >& nav, double nav_size) {
  double nav_div = (double)1 / nav_size / nav_size;
  int size = m.size();
  vector<double> v;
  for (int i = 0; i < size; i++) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    for (int j = 0; j < size; j++) {
      ...
      if (m[i][j] - nav[i][j] > 20) {
        ...
        tmp = 2 - ((double)1 / pow(2, m[i][j] - nav[i][j] - 1));
      }
      if (nav[i][j] != 0) {
        tmp += ((double)1 / pow(2, m[i][j] - 1)) * nav_div;
      }
      cout << tmp << '\t';
      v.push_back(tmp);
    }
...
/**
  ...
  @input: int nav_size        - 导航栏网页数量
  @input: int nav[size][size] - 导航栏网页指向关系
...
int main() {
  ...
  vector<double> v;
  vector<vector<double> > nav;
  cin >> size;
  scan(m, size);
  cin >> nav_size;
  scan(nav, size);
  createMatrix(m, nav, nav_size);
  for (int i = 0; i < size; i++) {
    v.push_back(static_cast<double>(1) / size);
  }
  cout << endl;
