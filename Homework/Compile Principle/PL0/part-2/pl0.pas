...
Type
  ...
  fct = (lit, opr, lod, sto, cal, int, jmp, jpc, rd, wr); {functions}
...
{JMP 0,a : 转移到指令地址a处}
{JPC 0,a : 条件转移到指令地址a处}
{RD  l,a : 读键盘数据存到层差为l的层、相对地址为a的变量}
{WR  0,0 : 写栈顶数据到屏幕}
...
Begin {statement}
  ...
  Else If sym = whilesym Then {处理循环语句}
    ...
    End
  Else If sym = readsym Then {处理输入语句}
    Begin
      getsym;
      If sym <> lparen Then error(19)
      Else
        Repeat
          getsym;
          If sym = ident Then {标识符}
            Begin
              i := position(id);
              If i = 0 Then error(11)
              Else If table[i].kind <> variable Then error(32)
              Else
                With table[i] Do
          ...
          getsym;
        Until sym <> comma;
      If sym <> rparen Then error(22); {右括号判断}
      getsym;
    End
  Else If sym = writesym Then {处理输出语句}
    Begin
      getsym;
      If sym <> lparen Then error(19)
      Else
        ...
        jpc: {当前指令是(jpc, 0, a)}
          Begin
            If s[t] = 0 Then p := a;
              t := t-1
          End;
        rd: readln(s[base(l) + a]); {当前指令是(rd, l, a)}
        wr: {当前指令是(wr, 0, 0)}
          Begin
            writeln(s[t]);
            t := t+1
