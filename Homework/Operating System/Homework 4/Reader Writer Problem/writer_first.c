...
pthread_mutex_t writeCountMutex;
...
int main(int agrv, char* args[]) {
    ...
    pthread_mutex_init(&writeCountMutex, NULL);
    ...
    pthread_mutex_destroy(&writeCountMutex);
...
void* Writer(void* param) {
    ...
    pthread_mutex_lock(&writeCountMutex);
    writeCount++;
    if (writeCount == 1) {
        pthread_mutex_lock(&readMutex);
    }
    pthread_mutex_unlock(&writeCountMutex);
